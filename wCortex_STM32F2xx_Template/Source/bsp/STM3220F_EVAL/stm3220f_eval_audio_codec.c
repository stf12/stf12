/**
  ******************************************************************************
  * @file    stm3220f_eval_audio_codec.c
...
--------------------------
   - This driver supports STM32F2xx devices on STM3220F-EVAL (MB786 RevA) Evaluation boards.

   - Configure the options in file stm3220_eval_audio_codec.h in the section CONFIGURATION.
   ...
   - Call the function EVAL_AUDIO_Init(
      ...
      if the returned value is different from 0 or the function is stuck then the communciation with
      ...
      + OUTPUT_DEVICE_AUTO: Selection of output device is made through external switch (implemented 
         into the audio jack on the evalutation board). When the Headphone is connected it is used
   ...
   - Call the function EVAL_AUDIO_PauseResume(
       ...
       Note. This function should be called only when the audio file is palyed or paused (not stopped).

   - For each mode, you may need to implement the relative callback functions into your code.
      The Callback functions are named EVAL_AUDIO_XXX_CallBack() and only their prototypes are declared in 
      the stm3220f_eval_audio_codec.h file. (refer to the example for more details on the callbacks implementations)
   ...
   - The driver API and the callback functions are at the end of the stm3220f_eval_audio_codec.h file.
 

 Driver architecture:
 --------------------
 This driver is composed of three main layers:
   o High Audio Layer: consists of the function API exported in the stm3220f_eval_audio_codec.h file
   ...
   o Codec Control layer: consists of the functions API controlling the audio codec (CS43L22) and 
     included as local functions in file stm3220f_eval_audio_codec.c (Codec_Init(), Codec_Play() ...)
   o Media Access Layer (MAL): which consists of functions allowing to access the media containing/
     providing the audio file/stream. These functions are also included as local functions into
     the stm3220f_eval_audio_codec.c file (Audio_MAL_Init(), Audio_MAL_Play() ...)
...
---------------------
     ...
     + AUDIO_MAL_MODE_CIRCULAR: is suitable when the audio data are read either from a 
        meomory location or from a device at real time (double buffer could be used).
...
------------------------------
     ...
     + EVAL_AUDIO_IT_HT_ENABLE: Enable this define to use the DMA end of half transfer interrupt.
        then, a callback should be impelemented by user to perform specific actions
...
---------------------
   ...
   2- Communication with the audio codec (through I2C) may be corrupted if it is interrupted by some
      ...
      communciation then re-enabled when it is over). Note that this communication is only done at
...
/* Includes ------------------------------------------------------------------*/
#include "stm3220f_eval_audio_codec.h"
...
#define CODEC_RESET_DELAY               0x2FFF
